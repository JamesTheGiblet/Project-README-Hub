...
The Truth Decay Engine is part of a broader protocol that unifies knowledge processing, reproducible environments, and human-facing interfaces.

* **Truth Decay Engine** (*this project*) → Core knowledge/uncertainty layer. Produces **Decay Maps** and drives insight from ambiguity.
* **devenv** → Construct layer. Provides a reproducible, local-first runtime for deploying and interacting with the engine.
* **Mythic Builder Portal** → Interface layer. Surfaces APIs and knowledge artifacts through accessible, extensible user experiences.

Together, these projects form a cohesive creation stack: **Core (knowledge) → Construct (environment) → Interface (access).**
...
* **Philosopher-Engineers**: You translate epistemic frameworks and philosophical concepts into clean, functional code.
* **Metadata Cartographers**: You specialize in mapping degraded knowledge, finding new ways to make sense of uncertainty.
...
This project interoperates with **devenv** (for environment reproducibility) and the **Mythic Builder Portal** (for accessible interfaces). Each project remains independent, but all share the same guiding values: speed, truth, and anti-fragility.
